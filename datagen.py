...
IMAGE_W = 144
IMAGE_H = 192
IMAGE_DIR = '/Users/zachhilton/Downloads/Animal Sketches/Phylopic2'
...
for root, subdirs, files in os.walk(IMAGE_DIR):
	...
	for file in files:
		...
		img = yb_resize(img)
		for i in range(SAMPLES_PER_IMG): #used to be xrange, but it's deprecated
			y_data[ix] = np.transpose(img, (2, 0, 1)) #only if you want color

			x_data[ix] = rand_dots(img, i)
			if ix < SAMPLES_PER_IMG*16: #i believe this is just for debugging, because having those pictures seems worthless
				outimg = x_data[ix][0]
				cv2.imwrite('cargb' + str(ix) + '.png', outimg)
				print(path)
			ix += 1
			y_data[ix] = np.flip(y_data[ix - 1], axis=2) #the mirror image!! Thats why it skips along by twos
...
assert(ix == NUM_SAMPLES)
print("Saving...")
# np.save('x_dataSHADOW.npy', x_data)
np.save('y_dataSHADOW2.npy', y_data)