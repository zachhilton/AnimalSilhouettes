...
enc_input = Input(shape=(None, num_params))
x=enc_input
index =2 #the index of the 'encoder'

for i in range(index, len(enc_model.layers)):
	# freeze the layer
	enc_model.layers[i].trainable = False

	# connect the layer
	x = enc_model.layers[i](x)

model_output = x
enc = Model(enc_input, model_output)


print("Loading Statistics...")
means = np.load('meansSHADOW2.npy')
stds  = np.load('stdsSHADOW2.npy')
evals = np.sqrt(np.load('evalsSHADOW2.npy'))
evecs = np.load('evecsSHADOW2.npy')
...
pygame.display.set_caption('Animal Sketch')
...
def update_mouse_click(mouse_pos):
	...
	if x >= 0 and y >= 0 and x < sliders_w and y < sliders_h:
		...
		if (cur_slider_ix>=num_params):
			cur_slider_ix=num_params-1
		mouse_pressed = True
